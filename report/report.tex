...
\begin{abstract}
As LLM's become increasingly more integrated into the lives of youths,the rapid evolution of Gen Z and Gen Alpha slang has created a critical misalignment between NLP systems and their primary demographic. Current normalization models often rely on opaque "black-box" architectures that lack interpretability. We hypothesize that the semantic shift between youth slang and standard English is not arbitrary but defined by identifiable, low-dimensional directions in embedding space, which we term the "\textit{Eigenslang}." We evaluate this hypothesis using a curated dataset of 96 slang-neutral pairs, comparing mean vector difference and PCA-based against raw retrieval baselines. Our results provide only weak evidence for a single shared slang direction, with the first principal component explaining only 5\% of variance.
An abbreviation-filtered ablation improves raw retrieval substantially, which suggests that dataset heterogeneity matters, but it still does not produce strong evidence for a single global PCA direction. We conclude that while representation choice significantly impacts performance, a single global direction is an incomplete explanation for the complex nature of slang normalization.
\end{abstract}

\section{Introduction}
Modern NLP systems still struggle with youth slang, especially with the consistently evolving Gen Z and Gen Alpha slang prevalent on social media. Common Gen Z expressions such as \textit{mid}, \textit{cooked}, \textit{ate}, or \textit{rizz} are recent, informal, and context-dependent, so standard lexical resources and embeddings often represent them poorly. Despite strategies such as subword tozenization, contextual embeddings, and diverse training dataset, challenges remain due to the fluid and rapidly evolving nature of slang \citep{ishita-mamidi-2025-evolution}. Furthermore, existing normalization frameworks, such as Neural Machine Translation (NMT) systems, typically rely on "black-box" encoder-decoder architectures. These models lack interpretability, providing no clear insight into the semantic transformations they apply to informal text. This is detrimental for tasks such as text understanding, retrieval, sentiment analysis, and chatbot interaction. Ultimately, the inability of these systems to accurately and transparently parse slang creates a fundamental disconnect in user experience and accessibility for the demographic that uses LLMs most frequently.
...
Second, our project is motivated by recent advancements in identifying interpretable directions within embedding spaces. \citet{shin2018eigenvectors} demonstrate that the eigenvectors of word-embedding spaces can uncover latent semantic structures that are otherwise obscured. More recently, \citet{shaik2026redefining} emphasize the necessity of moving beyond opaque model outputs, proposing the interpretable decomposition of language models to mitigate toxicity by identifying specific components responsible for problematic content.
...
\section{Experiments}
\subsection{Setup}
We split the cleaned dataset into train, validation, and test sets with a deterministic 60/20/20 split. We evaluate raw retrieval, mean offset, and PCA Eigenslang under both representation modes and both embedding techniques. The evaluation is conducted by applying the computed transformation (the "candidate Eigenslang") to the neutral word embeddings within the test split. We then measure the success of this reconstruction using \textbf{Top-1} and \textbf{Top-3} retrieval accuracy:

\textbf{Top-1 Accuracy}: The percentage of cases where the target slang term is the nearest neighbor to the transformed neutral vector in the embedding space.

\textbf{Top-3 Accuracy}: The percentage of cases where the target slang term appears within the three closest neighbors.

This approach allows us to quantify the degree to which a linear vector shift can accurately map neutral language to its youth-slang counterparts.
...
The contextual sentence formulation only shows a small PCA gain.
...
\bibitem[Shaik et~al.(2026)Shaik, Mazhar, Srivastava, and Akhtar]{shaik2026redefining}
...
\newblock In \emph{The Thirty-ninth Annual Conference on Neural Information Processing Systems}.
